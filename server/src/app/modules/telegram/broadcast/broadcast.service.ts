...
export class BroadcastService {
  ...
  private parseMessageForButtons(message: Message): ParsedBroadcastMessage {
    ...
    try {
      ...
      this.logger.warn(
        `Failed to parse buttons from message: ${error.message}, text: "${text}"`
      );
      return {
        cleanMessage: text,
        buttons: [],
  ...
  private listenToQueueEvents(): void {
    this.queue.on(
      ...
      (
        ...
        result: 'sent' | 'blocked' | 'retry_scheduled'
      ) => {
  ...
  private async checkProgress(
    ...
    this.logger.debug(
      `Broadcast ${broadcastId} progress: processed=${processed}/${report.total}, active_retries=${activeRetries}, sent=${report.sent}, failed=${report.failed}, blocked=${report.blocked}`
    );
    ...
    if (processed === report.total && activeRetries === 0) {
      this.logger.log(
        `All tasks completed for broadcast ${broadcastId}, finalizing...`
      );
      await this.finalizeBroadcast(report, broadcastId);
    } else if (processed === report.total && activeRetries > 0) {
      this.logger.log(
        `Main tasks completed for broadcast ${broadcastId}, but ${activeRetries} retry tasks still active`
      );
  ...
  private async sendMainTasksCompletedNotification(
    report: BroadcastReport,
...
Финальный отчет будет отправлен после завершения всех повторных попыток.`;
    ...
    try {
      ...
      this.logger.error(
        `Failed to send main tasks completed notification for broadcast to admin ${report.adminId}`,
        error
      );
    }
  }
  ...
  private async finalizeBroadcast(
    ...
    try {
      ...
      this.logger.error(
        `Failed to save broadcast report ${broadcastId}`,
        error
      );
    }
...
Всего: ${report.total}`;
    ...
    try {
      ...
      this.logger.error(
        `Failed to send final report for broadcast ${broadcastId} to admin ${report.adminId}`,
        error
      );
    }
...
Ошибок: ${report.failed}`;
    ...
    try {
      ...
      this.logger.error(
        `Failed to send progress update for broadcast to admin ${report.adminId}`,
        error
      );
    }
  }
  ...
  private async queueUsers(
    ...
    try {
      ...
      for (;;) {
        const tgIds = await this.usersService.getIdsBatch(
          offset,
          BROADCAST_CONFIG.BATCH_SIZE
        );
        ...
        this.logger.log(
          `Queued batch of ${tgIds.length} users for broadcast ${broadcastId} (total: ${totalQueued})`
        );
      }
      ...
      this.logger.error(
        `Failed during user queuing for broadcast ${broadcastId}`,
        error
      );
      report.status = 'failed';
    }
  ...
  async scheduleRetry(jobData: BroadcastJob, delayMs: number): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Scheduled retry for user ${jobData.tgId} in broadcast ${broadcastId} with delay ${delayMs}ms`
      );
    } catch (error) {
      this.logger.error(
        `Failed to schedule retry for user ${jobData.tgId} in broadcast ${broadcastId}`,
        error
      );
  ...
  async loadBroadcastReport(
    ...
    try {
      ...
      this.logger.error(
        `Failed to load broadcast report ${broadcastId}:`,
        error
      );
      return null;
    }
  ...
  async startRetryBroadcast(
    ...
    if (!originalReport.errors || originalReport.errors.length === 0) {
      throw new Error(
        `В рассылке ${originalBroadcastId} нет пользователей с ошибками`
      );
    }
    ...
    const message = Array.isArray(originalReport.message)
      ? originalReport.message[0]
    ...
    try {
      ...
      this.logger.error(
        `Failed to start retry broadcast ${retryBroadcastId}`,
        error
      );
      report.status = 'failed';
      throw error;
   ...
   */
  private async queueUsersWithErrors(
    originalReport: BroadcastReport,
    retryBroadcastId: string,
    retryReport: BroadcastReport
  ): Promise<void> {
    try {
      // Получаем сообщение, обрабатывая случай когда это может быть массив
      const message = Array.isArray(originalReport.message)
      ...
      if (activeTgIds.length === 0) {
        this.logger.warn(
          `No active users found for retry broadcast ${retryBroadcastId}`
        );
      ...
      this.logger.log(
        `Queued ${activeTgIds.length} users for retry broadcast ${retryBroadcastId} (original broadcast: ${originalReport.startTime})`
      );
    } catch (error) {
      this.logger.error(
        `Failed during user queuing for retry broadcast ${retryBroadcastId}`,
        error
      );
